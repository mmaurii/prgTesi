...
var Extractor = /** @class */ (function () {
    function Extractor() {
        ...
        this.annotatedCodeFilePath = "./annotatedCode/inputNested.ts";
     ...
     */
    Extractor.prototype.getFunctionInvoked = function (index, fnName, params) {
    ...
    Extractor.prototype.paramsInjection = function (fnName, params) {
        //check if the function has the right number of parameters
        var fnData = this.miniSLFunctionCode.get(fnName);
        if (fnData.params.length !== params.length) {
            throw new Error("Invalid number of parameters passed to function ".concat(fnName));
        }
        ...
        var miniSLCode = fnData.code;
        for (var i = 0; i < fnData.params.length; i++) {
            var regex = new RegExp("".concat(fnData.params[i], "(?!\\()"), "g");
            ...
            if (endIndex === -1) {
                ...
                try {
                    ...
                    if (ann === this.config.controlStatements.for) {
                        ...
                        if (!paramsArray.every(function (param) { return param.trim().match(/^[a-zA-Z_]*[a-zA-Z0-9_]*$/); })) {
        ...
        for (var i = index; i < this.annotations.length && this.endOfAnnotation; i++) {
            ...
            if (endIndex !== -1) {
                ...
                if (annotation.startsWith(this.config.controlStatements.function)) {
                    //check if the guard is well formed
                    var params = unspacedAnn.substring(startIndex, unspacedAnn.length - 1).split(",");
                    if (!params.every(function (param) { return param.match(/^[a-zA-Z_]*[a-zA-Z0-9_]*$/); })) {
     ...
     * @throws Error if the function is not found in the map or in the annotations array
     */
    Extractor.prototype.readFunctionAnnotations = function (i, annotation, params) {
